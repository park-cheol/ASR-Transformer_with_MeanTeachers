...
def get_args():
    ...
    parser.add_argument('--train-file', type=str, default='../dataset/train.json')
...
def load_noise_files(path):
    ...
    for file in name:
        fuu = os.path.join(path, file)
        # print(fuu)
        files = os.listdir(fuu)
        for name in files:
            if not name.endswith('wav'):
                # print(name)
                continue
            full = os.path.join(fuu, name)
            list.append(full)
...
if __name__ == '__main__':
    ...
    for index in range(len(trainData_list)):
        ...
        clean_amp, _ = librosa.load(audio_path, sr=16000)
        print(audio_path)

        noise_list = load_noise_files(args.noise_file_path)
        noise_wav_name = random.choice(noise_list)
        print("noise_wav_name: ", noise_wav_name)
        # noise_wav = wave.open(noise_wav_name, "r")
        noise_amp, _ = librosa.load(noise_wav_name, sr=16000)

        # clean_amp = cal_amp(clean_wav)
        # noise_amp = cal_amp(noise_wav)
        # print("clean: ", clean_amp.shape)
        # print("noise: ", noise_amp.shape)
        ...
        # 만약 clean_amp가 더 길 경우
        else:
            padding = np.zeros(clean_amp.shape)
            start = random.randint(0, len(clean_amp) - len(noise_amp))
            padding[start: start + len(noise_amp)] = padding[start: start + len(noise_amp)] + noise_amp
        ...
        sf.write(output_path, mixed_amp, 16000, subtype="PCM_24", format="WAV")
